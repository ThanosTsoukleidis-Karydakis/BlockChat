...
class Node:
    def __init__(self,port,ip):
        ...
        self.nonce=0
        self.ip=ip
        self.chain=Blockchain()  # private copy of the blockchain
        self.wallet=self.generate_wallet()  # wallet=[private_key, public key]
        self.stakes=[1,1,1,1,1]  # stakes of the nodes
        self.ring=[]  # state : info about other nodes (id, port, ip, balance, public key)
        self.id=-1
        self.nonces={'0':1, '1':1001,'2':2001, '3':3001, '4':4001}  # the nonce of the transaction that I expect from any other node
    ...
    # create a new transaction with the correct nonce and increase private nonce
    def create_transaction(self,receiver_address,type_of_transaction,content):
        ...
        print(transaction.message,transaction.type_of_transaction,transaction.amount)
        return transaction

    # validate a new transaction : check if the sender has enough coins to make this transaction,
    # verify the signature (=> verify the validity of the sender) and check if the nonce is the expected one
    def validate_transaction(self,message,amount,Signature, typeTrans,sender, nonce):
        ...
        print('TRANSACTION TO BE VALIDATED: ',message,' , WITH NONCE ', nonce, ' AND SELF.NONCES=',self.nonces)
        print("Is the sender able to make this transaction ? ",valid)
        
        validNonce=(nonce==self.nonces[id])
        if(validNonce):
            self.nonces[id]=self.nonces[id]+1
        
        print("Is the nonce valid ? ",validNonce)

        return PKCS1_v1_5.new(sender_public_key).verify(messageHash, base64.b64decode(Signature)) and valid and validNonce

        #use of signature and NBCs balance

    # bootstraping node handles genesis transaction
    def handleGenesisTransaction(self,trans):
        amount=trans.amount
        self.BCC=self.BCC+amount
        return self.BCC

    # add a new transaction into a block (to be used in order to add transactions in the current block of each node)
    def add_transaction_to_block(self,block, transaction):
        if(len(block.listofTransactions)<block.capacity):
            block.add_transaction(transaction)
        # the else condition is checked by the Users (User#.py)

    # compute the validator of a block
    ...
    # check if the prevHash of the block is the expected and check if the block was sent by the elected validator
    ...
    # and the node's balance if needed. Also it updates the stakes.
    def update_status(self, transaction, noFee):
            ...
            # if I don't know the transaction's receiver, it is a coin transaction to a node during the bootstraping.
            match_addr=False
            for node in self.ring:
            ...
            if transaction.sender_address==transaction.receiver_address:
                ...
                if transaction.type_of_transaction=="message":
                    print('UPDATE STATUS : I COUNTED ',len(transaction.message), ' CHARACTERS FOR ', transaction.message,'\n')
                    for node in self.ring:
                        if node["publickey"]==transaction.sender_address:
